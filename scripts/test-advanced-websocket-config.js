...
const EXPECTED_URL =
  'wss://vitalsense-websocket-advanced-dev.andernet.workers.dev/ws';
...
async function testHealthEndpoint() {
  try {
    const response = await fetch(
      'https://vitalsense-websocket-advanced-dev.andernet.workers.dev/health'
    );
    if (response.ok) {
...
async function main() {
  ...
  const configurations = [
    {
      ...
      test: () =>
        readPlistValue('ios/VitalSense/Resources/Config.plist', 'WS_URL'),
    },
    {
    ...
    },
    {
      ...
      test: () =>
        readPlistValue(
          'ios/VitalSense/Resources/Config.production.plist',
          'WS_URL'
        ),
    },
    {
      ...
      path: 'wrangler.toml',
      test: () =>
        readTomlValue('wrangler.toml', 'env.development.vars', 'WEBSOCKET_URL'),
    },
    {
      name: 'Wrangler Production',
      path: 'wrangler.toml',
      test: () =>
        readTomlValue('wrangler.toml', 'env.production.vars', 'WEBSOCKET_URL'),
    },
  ];
  ...
  for (const config of configurations) {
    ...
    if (url === EXPECTED_URL) {
      ...
      console.log(
        `❌ ${config.name}: ${url || 'NOT FOUND'} (expected: ${EXPECTED_URL})`
      );
      allCorrect = false;
    }
  }
  ...
  if (allCorrect && serviceAvailable) {
    ...
    console.log(
      '🚀 VitalSense iOS app is now configured to use Advanced ML WebSocket service'
    );
    console.log(
      '📱 Features available: Predictive Analytics, Anomaly Detection, Personalized Insights, Emergency Detection'
    );
  } else {
    console.log('⚠️  Some configurations may need attention');
    if (!allCorrect)
      console.log('   - Configuration files have incorrect URLs');
    if (!serviceAvailable)
      console.log('   - Service is not responding to health checks');
  }
}
