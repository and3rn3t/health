...
async function testMockMLResponses() {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      console.log(`⏰ Test timeout in phase: ${testPhase}, received ${responses.length} messages`);
      ws.close();
      reject(new Error(`Test timeout after receiving ${responses.length} responses`));
    }, 15000);
      ...
      try {
        ...
        if (message.type === 'connection_established') {
          ...
          resolve({
            ...
            phases: ['identification', 'ml_processing', 'predictions', 'insights']
          });
        }
