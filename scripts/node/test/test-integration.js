    ...
    for (const test of workerTests) {
      ...
      for (const endpoint of endpoints) {
        ...
        try {
          ...
          this.log(
            `  ${icon} ${endpoint.description}: ${response.status}`,
            color
          );
        } catch (error) {
  ...
  async testWebSocketConnection() {
    ...
    for (const test of wsTests) {
      try {
        ...
        const infoUrl =
          test.url.replace('ws://', 'http://').replace(':3001', ':8788') +
          '/ws';
        ...
        if (response.status === 200) {
          ...
          this.log(
            `❌ ${test.name}: WebSocket info failed (${response.status})`,
            'red'
          );
          this.results.summary.failed++;
        }
  ...
  async testDataFlow() {
    ...
    try {
      ...
      if (getResponse.status === 200) {
        ...
        this.log(
          `  ❌ Health data retrieval: Failed (${getResponse.status})`,
          'red'
        );
        this.results.summary.failed++;
      }
      ...
      if (!workerUrl.includes('workers.dev')) {
        ...
        if (postResponse.status >= 200 && postResponse.status < 300) {
          ...
          this.log(
            `  ❌ Health data submission: Failed (${postResponse.status})`,
            'red'
          );
          this.results.summary.failed++;
        }
  ...
  printSummary() {
    ...
    if (failed === 0) {
      ...
      if (failedPorts.length > 0) {
        this.log(
          `  📡 Start servers for ports: ${failedPorts.map((t) => t.port).join(', ')}`,
          'yellow'
        );
      }
    }
