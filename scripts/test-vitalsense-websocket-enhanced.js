...
const ws = new WebSocket('wss://vitalsense-websocket-enhanced-dev.andernet.workers.dev/ws');
...
ws.on('message', (data) => {
  try {
    ...
    if (message.type === 'vitalsense_connection_established') {
      console.log('🏥 VitalSense Enhanced features available:', message.data.capabilities);
    }
    
    if (message.type === 'vitalsense_live_health_update') {
      console.log('📊 Health metrics processed:', message.data.metrics?.length || 0);
      ...
      if (message.data.wellness_insights) {
        console.log('💡 Wellness score:', message.data.wellness_insights.overall_wellness_score);
      }
    }
...
ws.on('close', (code, reason) => {
  console.log(`📴 VitalSense WebSocket connection closed: ${code} ${reason || ''}`);
  process.exit(0);
});
