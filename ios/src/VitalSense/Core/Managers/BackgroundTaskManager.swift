...
class HealthDataSyncOperation: Operation {
    override func main() {
        ...
        Task {
            ...
            let healthData = await healthManager.fetchLatestHealthData()
            ...
            if isConnected && !healthData.isEmpty {
                // Send health data via API client
                for data in healthData {
                    _ = await ApiClient.shared.sendHealthData(data)
                }
            } else {
