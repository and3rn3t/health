...
class AdvancedHealthAnalytics: ObservableObject {
    ...
    func generateComprehensiveAnalysis() async {
        // Run analytics off main thread - UI updates will happen on main thread
        await withTaskGroup(of: Void.self) { group in
            group.addTask {
                guard !Task.isCancelled else { return }
                await self.analyzeHeartRatePatterns()
            }
            group.addTask {
                guard !Task.isCancelled else { return }
                await self.analyzeActivityPatterns()
            }
            group.addTask {
                guard !Task.isCancelled else { return }
                await self.analyzeSleepQuality()
            }
            group.addTask {
                guard !Task.isCancelled else { return }
                await self.generateHealthPredictions()
            }
            group.addTask {
                guard !Task.isCancelled else { return }
                await self.findHealthCorrelations()
            }
        }
    }
