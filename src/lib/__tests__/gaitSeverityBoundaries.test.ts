...
describe('gait severity boundaries', () => {
  ...
  it('increasing pattern shows non-decreasing severity ordering', () => {
    ...
    const s1 = severityNumeric(mildish.severity);
    const s2 = severityNumeric(moderateish.severity);
    const s3 = severityNumeric(strongish.severity);
  ...
  it('declining pattern yields negative severity that grows in magnitude', () => {
    ...
    const sd1 = severityNumeric(mildDecline.severity);
    const sd2 = severityNumeric(strongerDecline.severity);
