...
export function extractBuildings(
  ...
  function isBuildingCell(row: number, col: number): boolean {
    if (row < 0 || row >= chm.height || col < 0 || col >= chm.width)
      return false;
  ...
  function floodFill(
    ...
    bbox: { minRow: number; minCol: number; maxRow: number; maxCol: number };
  } {
  ...
  for (let row = 0; row < chm.height; row++) {
    for (let col = 0; col < chm.width; col++) {
      ...
      const heights = cells
        .map((c) => chm.grid[c.row]![c.col]!)
        .filter(Number.isFinite);
      const avgHeight = heights.reduce((a, b) => a! + b!, 0) / heights.length;
      const maxHeight = Math.max(...(heights as number[]));
      const minHeightInFeature = Math.min(...(heights as number[]));
...
export function extractTrees(
  ...
  function isTreeCell(row: number, col: number): boolean {
    ...
    const height = chm.grid[row]![col];
    return (
      Number.isFinite(height) && height! >= minHeight && height! <= maxHeight
    );
  }
  ...
  for (let row = 0; row < chm.height; row++) {
    for (let col = 0; col < chm.width; col++) {
      ...
      const heights = cells
        ...
        .filter(Number.isFinite);
      const avgHeight = heights.reduce((a, b) => a! + b!, 0) / heights.length;
      const maxHeightInFeature = Math.max(...(heights as number[]));
      const minHeightInFeature = Math.min(...(heights as number[]));
      const heightStd = Math.sqrt(
        heights.reduce((sum, h) => sum! + (h! - avgHeight) ** 2, 0) /
          heights.length
      );
      ...
      const confidence = Math.min(
        1.0,
        0.5 +
          (heightVariation > 0.1 ? 0.2 : 0) +
          (area > 10 && area < 200 ? 0.3 : 0)
      );
