...
export class SensorIntegrationTest {
  ...
  async runAllTests(): Promise<{ success: boolean; results: typeof this.testResults; errors: string[] }> {
    const errors: string[] = [];

    try {
      // Test 1: Sensor Permission Request
      console.log('🧪 Testing sensor permissions...');
      const hasPermissions = await DeviceSensorManager.requestSensorPermissions();
      this.testResults.permissionTest = true;
      console.log(`✅ Permission test: ${hasPermissions ? 'Granted' : 'Denied/Not Required'}`);
    } catch (error) {
    ...
    try {
      ...
      console.log(`✅ Initialization test: Sensors ${isSupported ? 'supported' : 'not supported'}`);
    } catch (error) {
    ...
    try {
      ...
      const success = await this.sensorManager.startAnalysis({
        onGaitUpdate: (metrics: GaitMetrics) => {
          receivedUpdate = true;
          console.log(`📊 Received gait update: Speed ${metrics.speed.toFixed(2)} m/s, Cadence ${metrics.cadence} spm`);
        },
      ...
      if (success) {
        ...
        console.log('⚠️ Analysis failed to start (may be expected in non-mobile environment)');
      }
    ...
    try {
      ...
      console.log(`✅ Error handling test: ${errorResult ? 'Unexpected success' : 'Handled gracefully'}`);
    } catch (error) {
    ...
    console.log('\n📋 Test Summary:');
    console.log('=================');
    console.log(`Permission Test: ${this.testResults.permissionTest ? '✅' : '❌'}`);
    console.log(`Initialization Test: ${this.testResults.initializationTest ? '✅' : '❌'}`);
    console.log(`Gait Analysis Test: ${this.testResults.gaitAnalysisTest ? '✅' : '❌'}`);
    console.log(`Error Handling Test: ${this.testResults.errorHandlingTest ? '✅' : '❌'}`);
  ...
  static testSensorAvailability(): {
    ...
    console.log(`DeviceOrientationEvent: ${results.deviceOrientation ? '✅' : '❌'}`);
    console.log(`Generic Accelerometer: ${results.accelerometer ? '✅' : '❌'}`);
...
if (typeof window !== 'undefined') {
  ...
  console.log('💡 Tip: Run testVitalSenseSensors() in the browser console to test sensor integration');
}
