...
export class HealthDataProcessor {
  private static generateTimeSeriesData(days: number, baseValue: number, variance: number, trend: number = 0): HealthMetric[] {
  ...
  private static calculateTrend(data: HealthMetric[]): 'increasing' | 'decreasing' | 'stable' {
  ...
  private static aggregateToWeekly(daily: HealthMetric[]): HealthMetric[] {
    ...
    for (let i = 0; i < daily.length; i += 7) {
      ...
      if (weekData.length > 0) {
        const weekAvg = weekData.reduce((sum, day) => sum + day.value, 0) / weekData.length
        weekly.push({
  ...
  private static processMetric(daily: HealthMetric[], metricName: string): MetricData {
    ...
    const percentileRank = (sortedValues.filter(v => v <= lastValue).length / sortedValues.length) * 100
  ...
  private static generateInsights(metrics: ProcessedHealthData['metrics']): string[] {
    const insights: string[] = []
    
    // Walking steadiness insights
    if (metrics.walkingSteadiness.average < 60) {
      insights.push(`Walking steadiness is below average (${Math.round(metrics.walkingSteadiness.average)}%). Consider balance exercises and consulting your healthcare provider.`)
    } else if (metrics.walkingSteadiness.trend === 'decreasing') {
      insights.push(`Walking steadiness has declined by ${Math.round(metrics.walkingSteadiness.variability)}% recently. Monitor for changes and consider preventive measures.`)
    }
    
    // Activity insights
    if (metrics.steps.average < 5000) {
      insights.push(`Daily step count is below recommended levels. Aim for gradual increases in physical activity.`)
    } else if (metrics.steps.trend === 'increasing') {
      insights.push(`Great progress! Your activity levels have increased by ${Math.round((metrics.steps.lastValue - metrics.steps.average) / metrics.steps.average * 100)}% recently.`)
    }
    
    // Heart rate insights
    if (metrics.heartRate.variability > 15) {
      insights.push(`Heart rate shows high variability. This could indicate stress or irregular patterns worth discussing with your doctor.`)
    }
    
    // Sleep insights
    if (metrics.sleepHours.average < 7) {
      insights.push(`Sleep duration is below recommended 7-9 hours. Poor sleep can affect balance and increase fall risk.`)
    }
    
    // Correlation insights
    if (metrics.steps.trend === 'increasing' && metrics.sleepHours.trend === 'increasing') {
      insights.push(`Your increased activity appears to correlate with better sleep quality - an excellent positive cycle!`)
    }
    
    return insights
  }

  private static calculateFallRiskFactors(metrics: ProcessedHealthData['metrics']): FallRiskFactor[] {
    ...
    if (metrics.walkingSteadiness.average < 50) {
      factors.push({
        ...
        risk: 'high',
        impact: 40,
        recommendation: 'Consult with a physical therapist for balance training exercises'
      })
      ...
      factors.push({
        ...
        risk: 'moderate',
        impact: 25,
        recommendation: 'Practice daily balance exercises and consider tai chi classes'
      })
    }
    ...
    if (metrics.steps.average < 3000) {
      factors.push({
        ...
        risk: 'high',
        impact: 30,
        recommendation: 'Gradually increase daily movement with safe, supervised activities'
      })
    ...
    if (metrics.heartRate.variability > 20) {
      factors.push({
        ...
        risk: 'moderate',
        impact: 20,
        recommendation: 'Monitor cardiovascular health and consult with your physician'
      })
    }
  ...
  private static calculateDataQuality(metrics: ProcessedHealthData['metrics']): ProcessedHealthData['dataQuality'] {
    ...
    const lastDataDate = new Date(metrics.steps.daily[metrics.steps.daily.length - 1]?.date || 0)
  ...
  private static calculateHealthScore(metrics: ProcessedHealthData['metrics'], fallRiskFactors: FallRiskFactor[]): number {
    let score = 100
    
    // Deduct points for fall risk factors
    const totalRiskImpact = fallRiskFactors.reduce((sum, factor) => sum + factor.impact, 0)
  ...
  static async processHealthData(file?: File): Promise<ProcessedHealthData> {
    ...
    const processedMetrics = {
      ...
      walkingSteadiness: this.processMetric(walkingSteadiness, 'walkingSteadiness'),
    ...
    const healthScore = this.calculateHealthScore(processedMetrics, fallRiskFactors)
