...
import { applySecurityHeaders, corsHeaders, getAesKey, encryptJSON, log, validateBearerJWT, verifyJwtWithJwks, writeAudit, decodeJwtPayload } from '@/lib/security'
...
type Env = {
  ...
  HEALTH_STORAGE?: {
    put: (key: string, data: string | ReadableStream | ArrayBuffer, opts?: any) => Promise<any>
    get: (key: string, opts?: any) => Promise<{ body?: ReadableStream | null } | null>
    list: (opts?: { prefix?: string; limit?: number }) => Promise<{ objects: Array<{ key: string; uploaded?: string | Date }> }>
  };
  ASSETS: {
...
async function rateLimitDO(c: any, key: string, limit = 60, intervalMs = 60_000): Promise<boolean> {
  try {
...
function deriveRateLimitKey(c: any): string {
  ...
  const sub = token ? (decodeJwtPayload(token)?.sub as string | undefined) : undefined
...
async function requireAuth(c: any): Promise<boolean> {
  ...
  if (jwksUrl) {
    const check = await verifyJwtWithJwks(token, { iss: c.env.API_ISS, aud: c.env.API_AUD, jwksUrl })
    return check.ok
  }
  const check = await validateBearerJWT(token, { iss: c.env.API_ISS, aud: c.env.API_AUD })
  return check.ok
}
...
app.use('/api/*', async (c, next) => {
  ...
  if (!(await rateLimitDO(c, key))) return c.json({ error: 'rate_limited' }, 429)
...
app.get('/api/_selftest', async (c) => {
  if (c.env.ENVIRONMENT === 'production') return c.json({ error: 'not_available' }, 404)
  ...
  // JWT negative check
  try {
    const bogus = 'eyJhbGciOiJSUzI1NiJ9.eyJpc3MiOiJiYSIsImF1ZCI6ImF1ZCIsImV4cCI6MX0.signature'
    const jwks = c.env.API_JWKS_URL
    if (jwks) {
      const res = await verifyJwtWithJwks(bogus, { iss: 'ba', aud: 'aud', jwksUrl: jwks })
      results.jwt_jwks_negative = { ok: !res.ok }
    } else {
      results.jwt_claims_negative = { ok: !(await validateBearerJWT(bogus)).ok }
    }
  } catch (e) {
...
app.get('/api/_ratelimit', async (c) => {
  if (c.env.ENVIRONMENT === 'production') return c.json({ error: 'not_available' }, 404)
  const key = (new URL(c.req.url)).searchParams.get('key') || deriveRateLimitKey(c)
  try {
...
app.get('/api/_audit', async (c) => {
  ...
  try {
    const listing = await c.env.HEALTH_STORAGE.list({ prefix: 'audit/events/', limit: 1000 })
    const objects = (listing.objects || [])
      .sort((a, b) => (a.key < b.key ? 1 : -1))
      .slice(0, limit)

    if (!withBodies) {
      return c.json({ ok: true, count: objects.length, keys: objects.map(o => o.key) })
    }
...
app.post('/api/health-data', async (c) => {
  ...
  try {
    ...
    if (kv) {
      ...
      const payload = encKey ? await encryptJSON(encKey, parsed.data) : JSON.stringify(parsed.data)
  ...
  await writeAudit(c.env as any, { type: 'health_data_created', actor: 'api', resource: 'kv:health', meta: { type: parsed.data.type, correlationId: corr } })
  } catch (e) {
...
export class RateLimiter {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    const now = Date.now()
    const record = (await this.storage.get(key)) || { tokens: limit, last: now }
    ...
    return new Response(JSON.stringify({ ok: true, remaining: record.tokens }), { status: 200, headers: { 'content-type': 'application/json' } })
  }
}
