...
export function useUsageTracking(
  ...
  const [usagePatterns, setUsagePatterns] = useKV<UsagePattern[]>('usage-patterns', [])
  ...
  useEffect(() => {
    ...
    setUsagePatterns(currentPatterns => {
      const existingPattern = currentPatterns.find(p => p.feature === featureName)
      if (existingPattern) {
  ...
  const getCurrentUsage = () => {
    return usagePatterns.find(p => p.feature === featureName) || {
      feature: featureName,
      visits: 0,
      lastVisited: 0,
      timeSpent: 0,
