...
export default function CommunityShare({ healthData }: CommunityShareProps) {
  const [communityMembers, setCommunityMembers] = useKV<CommunityMember[]>('community-members', [])
  const [healthReports, setHealthReports] = useKV<HealthReport[]>('health-reports', [])
  ...
  const [selectedTab, setSelectedTab] = useState<'members' | 'reports' | 'settings'>('members')
    ...
    try {
      ...
      const report: HealthReport = {
        ...
        metrics: {
          ...
          fallRiskLevel: healthData.fallRiskFactors?.some(f => f.risk === 'high') ? 'high' : 
                        healthData.fallRiskFactors?.some(f => f.risk === 'medium') ? 'medium' : 'low',
  ...
  const shareReport = (reportId: string, memberIds: string[]) => {
    setHealthReports(current => 
      current.map(report => 
        report.id === reportId 
          ? { ...report, sharedWith: [...new Set([...report.sharedWith, ...memberIds])] }
          : report
      )
    )
  ...
  const getRelationshipColor = (relationship: string) => {
    switch (relationship) {
      ...
      case 'friend': return 'bg-orange-100 text-orange-800'
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-foreground">Community & Sharing</h2>
          ...
          {isAddingMember && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Invite family members, healthcare providers, or caregivers to view your health progress
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select 
                    ...
                    onValueChange={(value) => setNewMember(prev => ({ ...prev, relationship: value as any }))}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
              ...
              <Card key={member.id}>
                <CardContent className="p-4">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          {member.lastViewed && (
                            <span className="text-xs text-muted-foreground">
                              Last viewed {new Date(member.lastViewed).toLocaleDateString()}
                            </span>
                          )}
                        </div>
                      </div>
                  ...
                  <div className="mt-4 pt-4 border-t">
                    <h5 className="text-sm font-medium mb-2">Access Permissions</h5>
              ...
              <Card key={report.id}>
                <CardHeader>
                  <div className="flex items-start justify-between">
                    ...
                    <Badge variant="outline" className={getRiskColor(report.metrics.fallRiskLevel)}>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="text-center">
                      <div className="text-2xl font-bold text-primary">{report.metrics.healthScore}</div>
                      <div className="text-xs text-muted-foreground">Health Score</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-green-600">{report.metrics.stepCount.toLocaleString()}</div>
                      <div className="text-xs text-muted-foreground">Daily Steps</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-blue-600">{report.metrics.activeMinutes}</div>
                      <div className="text-xs text-muted-foreground">Active Minutes</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-purple-600">{report.metrics.sleepQuality}%</div>
                        ...
                        <div className="flex -space-x-2">
                          ...
                          {report.sharedWith.length > 3 && (
                            <div className="w-6 h-6 bg-muted rounded-full border-2 border-background flex items-center justify-center">
                              <span className="text-xs">+{report.sharedWith.length - 3}</span>
                            </div>
                          )}
                        </div>
                      )}
            ...
            <CardContent className="space-y-6">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <Switch
                      ...
                      onCheckedChange={(checked) => 
                        setShareSettings(prev => ({ ...prev, autoShareWeekly: checked }))
                      }
                    />
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <Label htmlFor="emergency-share">Emergency Fall Alerts</Label>
                      <p className="text-sm text-muted-foreground">
                    ...
                    <Switch
                      ...
                      onCheckedChange={(checked) => 
                        setShareSettings(prev => ({ ...prev, emergencyAutoShare: checked }))
                      }
                    />
                  </div>
                </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <div>
                      <Label htmlFor="public-profile">Public Health Profile</Label>
                      <p className="text-sm text-muted-foreground">
                        Allow others to discover your health journey (anonymized)
                      </p>
                    </div>
                    <Switch
                      id="public-profile"
                      ...
                      onCheckedChange={(checked) => 
                        setShareSettings(prev => ({ ...prev, publicProfile: checked }))
                      }
                    />
                  </div>
                </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid md:grid-cols-3 gap-4">
                <div className="text-center">
                  <div className="text-2xl font-bold text-primary">{communityMembers.length}</div>
                  <div className="text-sm text-muted-foreground">Community Members</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-green-600">{healthReports.length}</div>
                  <div className="text-sm text-muted-foreground">Reports Shared</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-blue-600">
                    {healthReports.reduce((sum, report) => sum + report.viewCount, 0)}
                  </div>
