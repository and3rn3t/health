...
import { Heart, TrendingUp, TrendingDown, Activity, Brain, AlertTriangle, Clock, Target, Zap, Shield } from '@phosphor-icons/react'
...
export default function EnhancedHealthInsightsDashboard({ healthData }: Props) {
  ...
  const [predictiveAlerts, setPredictiveAlerts] = useKV<PredictiveAlert[]>('predictive-alerts', [])
  ...
  const generatePredictiveAlerts = async (): Promise<PredictiveAlert[]> => {
    ...
    if (healthData.healthScore && healthData.healthScore < 70) {
      alerts.push({
        ...
        prediction: 'Based on current trends, there is a 65% chance of further cardiovascular health decline within the next 2 weeks.',
        confidence: 65,
        timeframe: '2 weeks',
        preventiveActions: [
    ...
    if (healthData.fallRiskFactors?.some(factor => factor.risk === 'moderate' || factor.risk === 'high')) {
      alerts.push({
        id: 'fall-prediction',
        title: 'Increased Fall Risk Likelihood',
        prediction: 'Current gait and balance metrics suggest a 40% increased fall risk over the next month.',
        confidence: 72,
        timeframe: '1 month',
        preventiveActions: [
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card>
          ...
          <CardContent>
            ...
            <Progress value={healthData.healthScore || currentScore} className="mt-2" />
            <p className="text-sm text-muted-foreground mt-2">
              {(healthData.healthScore || currentScore) >= 80 ? 'Excellent' : 
               (healthData.healthScore || currentScore) >= 60 ? 'Good' : 'Needs Attention'}
            </p>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="space-y-2">
              {healthData.fallRiskFactors ? (
                <Badge variant={
                  healthData.fallRiskFactors.some(f => f.risk === 'high') ? 'destructive' :
                  healthData.fallRiskFactors.some(f => f.risk === 'moderate') ? 'secondary' : 'default'
                }>
                  {healthData.fallRiskFactors.some(f => f.risk === 'high') ? 'High Risk' :
                   healthData.fallRiskFactors.some(f => f.risk === 'moderate') ? 'Moderate Risk' : 'Low Risk'}
                </Badge>
              ) : (
                <Badge variant="outline">Not Assessed</Badge>
              )}
              <p className="text-sm text-muted-foreground">
                {healthData.fallRiskFactors?.length || 0} risk factors identified
              </p>
            </div>
          </CardContent>
        </Card>
      ...
      <Tabs defaultValue="insights" className="space-y-4">
        ...
        <TabsContent value="insights" className="space-y-4">
          ...
          {selectedInsight && selectedInsight.recommendations && (
            <Card>
              <CardHeader>
                <CardTitle>Recommendations for: {selectedInsight.title}</CardTitle>
              </CardHeader>
              <CardContent>
          ...
          {predictiveAlerts.map((alert) => (
            <Card key={alert.id}>
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  <h4 className="font-medium mb-2">Preventive Actions:</h4>
        ...
        <TabsContent value="recommendations" className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {insights.filter(insight => insight.actionable && insight.recommendations).map((insight) => (
              <Card key={insight.id}>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2 text-sm">
                ...
                <CardContent>
                  <div className="space-y-2">
                    {insight.recommendations?.map((rec, index) => (
                      <Button key={index} variant="outline" size="sm" className="w-full justify-start text-xs h-auto py-2">
                        {rec}
                      </Button>
                    ))}
                  </div>
