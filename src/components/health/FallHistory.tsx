...
import { AlertTriangle, Plus, MapPin, Clock, Activity } from '@phosphor-icons/react'
...
export default function FallHistory() {
  const [fallHistory, setFallHistory] = useKV('fall-history', [] as FallIncident[])
  ...
  const handleAddIncident = () => {
    if (!newIncident.location || !newIncident.fallType || !newIncident.injuryLevel) {
  ...
  const getInsights = () => {
    ...
    const mostCommonLocation = Object.entries(locationCounts).sort((a, b) => b[1] - a[1])[0]
    if (mostCommonLocation && mostCommonLocation[1] > 1) {
      insights.push(`Most falls occur in: ${mostCommonLocation[0]} (${mostCommonLocation[1]} incidents)`)
    }
    ...
    const severeInjuries = injuryLevels.filter(level => level === 'moderate' || level === 'severe').length
    if (severeInjuries > 0) {
      insights.push(`${severeInjuries} fall(s) resulted in moderate to severe injuries`)
    }
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Record and analyze fall incidents to identify patterns and improve safety
              </CardDescription>
            </div>
            <Dialog open={isAddingIncident} onOpenChange={setIsAddingIncident}>
              ...
              <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Document details about the fall to help identify patterns and improve prevention
                  </DialogDescription>
                </DialogHeader>
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select 
                      ...
                      onValueChange={(value) => setNewIncident({ ...newIncident, location: value })}
                    >
                      <SelectTrigger>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select 
                      ...
                      onValueChange={(value) => setNewIncident({ ...newIncident, injuryLevel: value })}
                    >
                      <SelectTrigger>
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="minor">Minor (bruises, scratches)</SelectItem>
                        <SelectItem value="moderate">Moderate (sprains, cuts)</SelectItem>
                        <SelectItem value="severe">Severe (fractures, head injury)</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                  
                  {newIncident.injuryLevel && newIncident.injuryLevel !== 'none' && (
                    <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <Textarea
                      ...
                      onChange={(e) => setNewIncident({ ...newIncident, circumstances: e.target.value })}
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center space-x-2">
                      <input
                        ...
                        onChange={(e) => setNewIncident({ ...newIncident, helpReceived: e.target.checked })}
                        className="w-4 h-4"
                      />
                    ...
                    <div className="flex items-center space-x-2">
                      <input
                        ...
                        onChange={(e) => setNewIncident({ ...newIncident, hospitalVisit: e.target.checked })}
                        className="w-4 h-4"
                      />
        ...
        <CardContent>
          {fallHistory.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-muted-foreground mb-2">No fall incidents recorded</p>
              <p className="text-sm text-muted-foreground">
                Recording fall incidents helps identify patterns and improve prevention strategies
              </p>
            </div>
          ) : (
            <div className="space-y-4">
              ...
              {fallHistory.map((incident) => (
                <div key={incident.id} className="border rounded-lg p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant={getInjuryBadgeVariant(incident.injuryLevel)}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    <div>
                      <span className="font-medium">Fall Type:</span> {incident.fallType}
                    </div>
                    {incident.injuryDescription && (
                      <div>
                        <span className="font-medium">Injury:</span> {incident.injuryDescription}
                      </div>
                    )}
                    {incident.circumstances && (
                      <div className="md:col-span-2">
                        <span className="font-medium">Circumstances:</span> {incident.circumstances}
                      </div>
                    )}
                    {incident.notes && (
                      <div className="md:col-span-2">
