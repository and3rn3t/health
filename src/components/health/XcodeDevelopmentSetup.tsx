...
export default function XcodeDevelopmentSetup() {
  const [setupSteps, setSetupSteps] = useKV<SetupStep[]>('xcode-setup-steps', [
    ...
    // Project Setup
    {
      ...
      instructions: [
        ...
        'Set Bundle Identifier (e.g., com.yourname.vitalsense)',
    ...
    },
    {
      ...
      description:
        'Set up Info.plist with required HealthKit usage descriptions',
      category: 'healthkit',
      priority: 'critical',
    ...
    // watchOS Setup
    {
      ...
      instructions: [
        ...
        'Set Product Name (e.g., "vitalsense Watch")',
  ...
  const calculateProgress = (category?: string) => {
    const relevantSteps = category
      ? setupSteps.filter((s) => s.category === category)
      : setupSteps;
  ...
  const criticalStepsRemaining = setupSteps.filter(
    (s) => s.priority === 'critical' && !s.completed
  ).length;
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Complete guide to setting up Xcode for iOS and watchOS development
            with HealthKit integration
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold">
                {setupSteps.filter((s) => s.completed).length}/
                {setupSteps.length}
              </div>
              <div className="text-muted-foreground text-sm">
                Steps Complete
          ...
          {criticalStepsRemaining > 0 && (
            <Alert className="border-red-200 bg-red-50">
              ...
              <AlertDescription>
                {criticalStepsRemaining} critical setup steps remaining.
                Complete these first for basic functionality.
              </AlertDescription>
            </Alert>
          )}
        </CardContent>
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <Badge variant="outline">
                {
                  getStepsByCategory(category.id).filter((s) => s.completed)
                    .length
                }{' '}
                / {getStepsByCategory(category.id).length} completed
              </Badge>
            </div>
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 space-y-4">
                        <div className="flex items-center justify-between">
                          <h4
                            className={`font-medium ${step.completed ? 'text-muted-foreground line-through' : ''}`}
                          >
                            {step.title}
                          </h4>
                          <div className="flex items-center gap-2">
                            <div
                              className={`h-2 w-2 rounded-full ${getPriorityColor(step.priority)}`}
                            />
                        ...
                        <div className="space-y-3">
                          ...
                          {step.troubleshooting &&
                            step.troubleshooting.length > 0 && (
                              <div>
                                <h5 className="mb-2 text-sm font-medium text-amber-600">
                                  🔧 Troubleshooting:
                                </h5>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex items-center gap-2">
                  <Circle className="h-3 w-3" />
                  <a
                    href="https://developer.apple.com/documentation/watchconnectivity"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-primary hover:underline"
                  >
