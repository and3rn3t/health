...
export const LiDARSocialInteractionAnalyzer = React.memo<LiDARSocialInteractionAnalyzerProps>(
  ({ onAnalysisComplete: _onAnalysisComplete, className = '' }) => {
    ...
    const [currentSession, setCurrentSession] = useState<SocialAnalysisSession | null>(null);
    const [currentMetrics, setCurrentMetrics] = useState<SocialInteractionMetrics | null>(null);

    // Participant Management
    const [detectedParticipants, setDetectedParticipants] = useState<ParticipantProfile[]>([]);

    // History and Persistence
    const [sessionHistory, setSessionHistory] = useKV<SocialAnalysisSession[]>(
    ...
    const generateSocialAnalysis = useCallback(
      async (
        ...
        if (metrics.movementSynchrony > 0.8) {
          insights.push('Excellent movement coordination between participants');
        ...
        return {
          ...
          insights,
          recommendations,
          confidence: Math.min(0.95, 0.75 + metrics.spatialCoordination * 0.2),
        };
      },
      []
    );
    ...
    const startSession = useCallback(async () => {
      if (detectedParticipants.length < 2) {
        alert('At least 2 participants required for social interaction analysis');
        return;
      }
    ...
    }, [config, detectedParticipants, selectedSessionType.name, completeSession, currentMetrics, updateSocialMetrics]);
    ...
    const updateConfig = useCallback((updates: Partial<SocialAnalysisConfig>) => {
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <div className="text-muted-foreground text-sm">
                        ...
                        <p>
                          Movement Pattern: {participant.movementPattern}
                        </p>
                      </div>
                    </div>
    ...
    const renderResultsContent = useCallback(() => {
      ...
      if (sessionHistory && sessionHistory.length > 0) {
        const averageScores = sessionHistory.length > 0 ? {
          interaction: sessionHistory.reduce((sum, s) => sum + s.overallInteractionScore, 0) / sessionHistory.length,
          caregiving: sessionHistory.reduce((sum, s) => sum + s.caregivingEffectiveness, 0) / sessionHistory.length,
          social: sessionHistory.reduce((sum, s) => sum + s.socialConnectedness, 0) / sessionHistory.length,
          safety: sessionHistory.reduce((sum, s) => sum + s.safetySupport, 0) / sessionHistory.length,
        } : null;
                  ...
                  <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                    {SESSION_TYPES.map((type) => (
                      <Button
                        key={type.id}
                        variant={
                          config.sessionType === type.id ? 'default' : 'outline'
                        }
                        onClick={() =>
                          updateConfig({
                            sessionType: type.id,
                ...
                <div className="md:grid-cols-2 grid grid-cols-1 gap-4">
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {['home', 'clinic', 'outdoor', 'facility'].map((env) => (
                        <Button
                          key={env}
                          variant={
                            config.environment === env ? 'default' : 'outline'
                          }
                          size="sm"
                          onClick={() =>
                            updateConfig({
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      <Button
                        variant={config.enablePrivacyMode ? 'default' : 'outline'}
                        size="sm"
                        onClick={() =>
                          updateConfig({
                      ...
                      <Button
                        variant={
                          config.enableRealTimeCoaching ? 'default' : 'outline'
                        }
                        size="sm"
                        onClick={() =>
                          updateConfig({
                  ...
                  <div>
                    <div className="mb-2 flex items-center justify-between">
                      ...
                      <span className="text-muted-foreground text-sm">
                        ...
                        {String(Math.floor(timeRemaining % 60)).padStart(2, '0')} remaining
                      </span>
                    </div>
                    ...
                    <div className="md:grid-cols-2 grid grid-cols-1 gap-4">
                      <Card>
                        <CardContent className="p-4">
                          <div className="mb-2 text-center">
                            <div className="text-blue-600 text-2xl font-bold">
                              {Math.round(currentMetrics.movementSynchrony * 100)}%
                            </div>
                            <div className="text-muted-foreground text-sm">
                              Movement Sync
                            </div>
                      ...
                      <Card>
                        <CardContent className="p-4">
                          <div className="mb-2 text-center">
                            <div className="text-green-600 text-2xl font-bold">
                              {Math.round(currentMetrics.supportBehaviors * 100)}%
                            </div>
                            <div className="text-muted-foreground text-sm">
                              Support Quality
                            </div>
