...
export default function FamilyDashboard({ healthData }: FamilyDashboardProps) {
  ...
  const [progressShares, setProgressShares] = useKV<ProgressShare[]>('progress-shares', [
    {
      id: '1',
      type: 'achievement',
    ...
    },
    {
      ...
      description: 'Your balance and stability metrics have improved this week',
      value: 72,
      previousValue: 65,
      unit: 'score',
  ...
  const activeFamilyMembers = familyMembers.filter(member => 
    new Date().getTime() - member.lastSeen.getTime() < 1000 * 60 * 60 * 24 // Active in last 24h
  ).length

  const totalReactions = progressShares.reduce((sum, share) => sum + share.reactions.length, 0)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-foreground">Family Dashboard</h2>
      ...
      <div className="grid md:grid-cols-3 gap-4">
        ...
        <Card>
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-green-600">{activeFamilyMembers}</div>
            <div className="text-sm text-muted-foreground">Active Today</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-purple-600">{totalReactions}</div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm text-muted-foreground">{healthData.healthScore || 0}/100</span>
              </div>
              <Progress value={healthData.healthScore || 0} className="h-2" />
              <p className="text-xs text-muted-foreground">
                {(healthData.healthScore || 0) >= 80 ? 'Excellent health status! Keep it up!' :
                 (healthData.healthScore || 0) >= 60 ? 'Good health with room for improvement' :
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <Badge variant={
                  ...
                  healthData.fallRiskFactors?.some(f => f.risk === 'medium') ? 'default' :
                  'secondary'
                }>
                  {healthData.fallRiskFactors?.some(f => f.risk === 'high') ? 'HIGH' :
                   healthData.fallRiskFactors?.some(f => f.risk === 'medium') ? 'MEDIUM' :
                   'LOW'}
                </Badge>
              </div>
              <p className="text-xs text-muted-foreground">
                ...
                <div className="text-right">
                  <div className="flex items-center gap-2 mb-1">
                    <div className={`w-2 h-2 rounded-full ${
                      new Date().getTime() - member.lastSeen.getTime() < 1000 * 60 * 60 ? 'bg-green-500' : 'bg-gray-400'
                    }`} />
              ...
              return (
                <div key={share.id} className="border rounded-lg p-4">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-3">
                      <div className={`p-2 rounded-full bg-gray-100 ${getTypeColor(share.type)}`}>
                  ...
                  {share.value && (
                    <div className="mb-3">
                      <div className="flex items-center gap-2">
                        <span className="text-2xl font-bold text-primary">{share.value}</span>
                        <span className="text-sm text-muted-foreground">{share.unit}</span>
                        {share.previousValue && (
