...
import { TrendingUp, TrendingDown, ArrowUp, ArrowDown, Activity, Heart, BarChart3, Zap, Clock, Target, AlertTriangle, CheckCircle, Info, Minus } from '@phosphor-icons/react'
...
export default function HealthInsightsDashboard({ healthData }: HealthInsightsDashboardProps) {
  const [trendHistory, setTrendHistory] = useKV<TrendData[]>('health-trend-history', [])
  const [liveInsights, setLiveInsights] = useKV<LiveInsight[]>('live-health-insights', [])
  ...
  const generateLiveInsights = async () => {
    ...
    try {
      ...
      if (healthData.metrics?.heartRate) {
        ...
        if (avgHR > 100) {
          insights.push({
            ...
            category: 'cardiovascular',
            actionable: true,
            recommendation: 'Consider relaxation techniques or consult your healthcare provider if persistent.'
          })
      ...
      if (healthData.metrics?.steps) {
        ...
        if (dailySteps < 8000) {
          insights.push({
            ...
            category: 'activity',
            actionable: true,
            recommendation: 'Try taking a 15-minute walk or using stairs instead of elevators.'
          })
      ...
      if (healthData.metrics?.sleep) {
        ...
        if (sleepHours < 7) {
          insights.push({
            ...
            category: 'recovery',
            actionable: true,
            recommendation: 'Consider establishing a consistent bedtime routine and avoiding screens before bed.'
          })
      ...
      if (healthData.fallRiskFactors && healthData.fallRiskFactors.length > 0) {
        const highRiskFactors = healthData.fallRiskFactors.filter(factor => factor.risk === 'high')
        if (highRiskFactors.length > 0) {
          insights.push({
            ...
            category: 'safety',
            actionable: true,
            recommendation: 'Review balance exercises and consider discussing with your healthcare provider.'
          })
        }
      }
      ...
      if (healthData.healthScore) {
        if (healthData.healthScore >= 80) {
          ...
          insights.push({
            ...
            category: 'overall',
            actionable: true,
            recommendation: 'Focus on improving sleep, activity levels, and monitoring key health metrics.'
          })
        }
      }
      ...
      setLiveInsights(currentInsights => {
        const filtered = currentInsights.filter(insight => 
          now - insight.timestamp < 24 * 60 * 60 * 1000 // Keep insights from last 24 hours
        )
  ...
  const calculateTrends = (): HealthTrend[] => {
    ...
    if (healthData.metrics.heartRate) {
      ...
      const historical = trendHistory.filter(t => t.metric === 'heartRate').slice(-10)
      const previous = historical.length > 0 ? historical[historical.length - 1]?.value || current : current
      ...
      trends.push({
        ...
        category: 'cardiovascular',
        unit: 'BPM',
        status: current >= 60 && current <= 80 ? 'good' : current > 100 ? 'warning' : 'neutral',
        insight: current >= 60 && current <= 80 ? 'Heart rate is in optimal range' : 
                current > 100 ? 'Heart rate is elevated - consider rest' : 'Heart rate is below normal range'
      })
    }
    ...
    if (healthData.metrics.steps) {
      ...
      const historical = trendHistory.filter(t => t.metric === 'steps').slice(-7) // Last 7 days
      const avgPrevious = historical.length > 0 ? 
        historical.reduce((sum, t) => sum + t.value, 0) / historical.length : current
      ...
      trends.push({
        ...
        category: 'activity',
        unit: 'steps',
        status: current >= 8000 ? 'good' : current < 5000 ? 'warning' : 'neutral',
        insight: current >= 10000 ? 'Excellent activity level!' : 
                current >= 8000 ? 'Good activity level' : 'Consider increasing daily activity',
        recommendation: current < 8000 ? 'Aim for at least 8,000 steps daily' : undefined
      })
    }
    ...
    if (healthData.metrics.sleep) {
      ...
      const historical = trendHistory.filter(t => t.metric === 'sleep').slice(-7) // Last 7 days
      const avgPrevious = historical.length > 0 ? 
        historical.reduce((sum, t) => sum + t.value, 0) / historical.length : current
      const change = current - avgPrevious
      const changePercent = avgPrevious > 0 ? (change / avgPrevious) * 100 : 0
      ...
      trends.push({
        ...
        category: 'recovery',
        unit: 'hours',
        status: current >= 7 && current <= 9 ? 'good' : current < 6 ? 'warning' : 'neutral',
        insight: current >= 7 && current <= 9 ? 'Sleep duration is optimal' : 
                current < 7 ? 'Consider improving sleep habits' : 'Sleep duration is above average',
        recommendation: current < 7 ? 'Aim for 7-9 hours of sleep nightly' : undefined
      })
    }
  ...
  const filteredInsights = selectedCategory === 'all' ? liveInsights : 
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-foreground">Health Insights Dashboard</h2>
          <p className="text-muted-foreground">Live trending data and personalized health insights</p>
        </div>
                ...
                <div className="space-y-2">
                  <div className="flex items-baseline gap-2">
                    <span className="text-2xl font-bold text-foreground">
                      {trend.metric === 'Sleep Duration' ? trend.current.toFixed(1) : Math.round(trend.current).toLocaleString()}
                    </span>
                    <span className="text-sm text-muted-foreground">{trend.unit}</span>
                  </div>
                  
                  <div className="flex items-center gap-2">
                    <Badge variant="outline" className={`text-xs ${getStatusColor(trend.status)}`}>
                      {trend.status === 'good' ? 'Optimal' : trend.status === 'warning' ? 'Needs Attention' : 'Monitor'}
                    </Badge>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            AI-powered insights and recommendations based on your current health data
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <div className="space-y-3">
              {filteredInsights.length === 0 ? (
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p className="text-sm">Click "Analyze Now" to generate insights</p>
                </div>
              ) : (
                filteredInsights
                  ...
                  .map((insight) => (
                    ...
                    >
                      <div className={`p-2 rounded-full ${
                        ...
                        insight.severity === 'success' ? 'bg-green-100' : 'bg-blue-100'
                      }`}>
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-2xl font-bold text-foreground">{healthData.healthScore}/100</span>
              </div>
              ...
              <div className="text-center">
                <p className="text-sm text-muted-foreground">
                  {healthData.healthScore >= 80 ? 'Excellent health! Keep up the great work.' :
                   healthData.healthScore >= 60 ? 'Good health with room for improvement.' :
