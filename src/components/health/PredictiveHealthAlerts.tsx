...
import { AlertTriangle, TrendingDown, Brain, Bell, Settings, Activity, Heart, Zap, Clock, Target, ChartLine, Warning, CheckCircle, XCircle } from '@phosphor-icons/react'
...
export default function PredictiveHealthAlerts({ healthData }: PredictiveHealthAlertsProps) {
  const [alerts, setAlerts] = useKV<HealthAlert[]>('predictive-health-alerts', [])
  ...
  const generateTrendAnalysis = async () => {
    ...
    try {
      ...
      for (const metric of metrics.slice(0, 6)) {
        ...
        if (slope < -0.1) direction = slope < -0.15 ? 'concerning' : 'declining'
      ...
      for (const trend of analysis) {
        if (trend.direction === 'concerning' || (trend.direction === 'declining' && trend.confidence > alertConfig.thresholds.confidence)) {
          ...
          if (decline >= alertConfig.thresholds.decline) {
            newAlerts.push({
              ...
              type: 'decline',
              severity: decline > 25 ? 'critical' : decline > 20 ? 'high' : 'medium',
  ...
  const generateRecommendations = (metric: string, decline: number): string[] => {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            AI-powered trend analysis and early warning system for health decline
          </p>
        </div>
        <Button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground">Metrics Tracked</p>
      ...
      {criticalAlerts.length > 0 && (
        <Alert className="border-destructive bg-destructive/10">
          ...
          <AlertDescription>
            <span className="font-semibold">Critical Health Alert:</span> {criticalAlerts.length} metric(s) showing concerning decline patterns. Immediate attention recommended.
          </AlertDescription>
        </Alert>
      )}
          ...
          {alerts.length === 0 ? (
            <Card>
              <CardContent className="p-8 text-center">
                ...
                <p className="text-muted-foreground">
                  Your health metrics are stable. Run analysis to check for new trends.
                </p>
              </CardContent>
            </Card>
          ) : (
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div>
                        ...
                        <p className="font-semibold">{alert.currentValue.toFixed(1)}</p>
                      </div>
                      <div>
                        <Label className="text-xs text-muted-foreground">Predicted Value</Label>
                        <p className="font-semibold text-destructive">{alert.predictedValue.toFixed(1)}</p>
                      </div>
                      <div>
                        <Label className="text-xs text-muted-foreground">Confidence</Label>
                        <div className="flex items-center gap-2">
                          <Progress value={alert.confidence * 100} className="flex-1" />
          ...
          {trendAnalysis.length === 0 ? (
            <Card>
              <CardContent className="p-8 text-center">
                ...
                <h3 className="text-lg font-semibold mb-2">No Trend Analysis Available</h3>
                <p className="text-muted-foreground">
                  Run analysis to generate ML-powered trend predictions for your health metrics.
                </p>
              </CardContent>
            </Card>
          ) : (
                ...
                <Card key={index}>
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      ...
                      <Badge variant={trend.direction === 'concerning' ? 'destructive' : trend.direction === 'improving' ? 'default' : 'secondary'}>
                        {trend.direction}
                      </Badge>
                    </div>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div>
                        <Label className="text-xs text-muted-foreground">Next Week Prediction</Label>
                        <p className="font-semibold">{trend.prediction.nextWeek.toFixed(1)}</p>
                      </div>
                      <div>
                        <Label className="text-xs text-muted-foreground">Next Month Prediction</Label>
                        <p className="font-semibold">{trend.prediction.nextMonth.toFixed(1)}</p>
                      </div>
                      <div>
                        <Label className="text-xs text-muted-foreground">Prediction Confidence</Label>
                        <div className="flex items-center gap-2">
                          <Progress value={trend.confidence * 100} className="flex-1" />
                          <span className="text-sm font-medium">{(trend.confidence * 100).toFixed(0)}%</span>
                        </div>
                      </div>
                    </div>
        ...
        <TabsContent value="settings" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Configure how predictive health alerts are generated and delivered
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="low">Low - Only major changes</SelectItem>
              ...
              <div>
                <Label className="text-base mb-4 block">Notification Preferences</Label>
