...
export default function RealTimeFallDetection() {
  ...
  const [calibrationData, setCalibrationData] = useKV<CalibrationData | null>('fall-detection-calibration', null)
  ...
  const generateMockSensorData = (): SensorData => {
    const baseAccel = calibrationData?.baseline.accelerometer || { x: 0, y: 0, z: -9.8 }
    const baseGyro = calibrationData?.baseline.gyroscope || { x: 0, y: 0, z: 0 }
  ...
  const detectFall = (sensorData: SensorData, history: SensorData[]): boolean => {
    ...
    const avgHeartRate = history.slice(-5).reduce((sum, data) => sum + (data.heartRate || 0), 0) / 5
  ...
  const handleFallDetected = (sensorData: SensorData, severity: 'low' | 'medium' | 'high' | 'critical') => {
  ...
  const cancelFallAlert = () => {
    if (alertPending) {
      setFallEvents(current => [...current, { ...alertPending, falseAlarm: true }])
    }
    setAlertPending(null)
    setCountdownTimer(null)
    if (countdownIntervalRef.current) {
  ...
  const startMonitoring = () => {
    ...
    sensorIntervalRef.current = setInterval(() => {
      ...
      const sensorData = shouldSimulateFall ? simulateFallEvent() : generateMockSensorData()
      ...
      const avgAccelMagnitude = recentSensorHistory.slice(-10).reduce((sum, data) => {
      ...
      {alertPending && countdownTimer !== null && (
        <Alert className="border-destructive bg-destructive/10">
          ...
          <AlertDescription className="flex items-center justify-between">
            <div>
              <strong>FALL DETECTED!</strong> Emergency contacts will be notified in {countdownTimer} seconds.
            </div>
            <Button 
              variant="destructive" 
              size="sm"
      ...
      <div className="grid md:grid-cols-3 gap-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex items-center gap-3">
              <Badge variant={
                fallRisk === 'high' ? 'destructive' : 
                fallRisk === 'medium' ? 'default' : 
                'secondary'
              }>
                {fallRisk.toUpperCase()}
              </Badge>
              {fallRisk === 'high' && <AlertTriangle className="h-4 w-4 text-destructive" />}
              {fallRisk === 'medium' && <Activity className="h-4 w-4 text-yellow-500" />}
              {fallRisk === 'low' && <CheckCircle className="h-4 w-4 text-green-500" />}
            </div>
            <p className="text-sm text-muted-foreground mt-2">
                ...
                <CardContent>
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      <span>X:</span>
                      <span className="font-mono">{currentSensorData.accelerometer.x.toFixed(2)} m/s²</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Y:</span>
                      <span className="font-mono">{currentSensorData.accelerometer.y.toFixed(2)} m/s²</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Z:</span>
                      <span className="font-mono">{currentSensorData.accelerometer.z.toFixed(2)} m/s²</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      <span>X:</span>
                      <span className="font-mono">{currentSensorData.gyroscope.x.toFixed(2)} rad/s</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Y:</span>
                      <span className="font-mono">{currentSensorData.gyroscope.y.toFixed(2)} rad/s</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Z:</span>
                      <span className="font-mono">{currentSensorData.gyroscope.z.toFixed(2)} rad/s</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
                ...
                <div className="h-64">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={chartData}>
                      ...
                      <YAxis />
                      <Tooltip />
                      <Line type="monotone" dataKey="accel" stroke="#3b82f6" name="Acceleration" strokeWidth={2} />
                      <Line type="monotone" dataKey="gyro" stroke="#ef4444" name="Rotation" strokeWidth={2} />
                      {calibrationData && (
                        <>
                          <ReferenceLine y={calibrationData.thresholds.impact} stroke="#f59e0b" strokeDasharray="5 5" />
                          <ReferenceLine y={calibrationData.thresholds.rotation} stroke="#ef4444" strokeDasharray="5 5" />
                        </>
                      )}
                    </LineChart>
                  </ResponsiveContainer>
        ...
        <TabsContent value="calibration" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Calibrate the fall detection system based on your baseline movement patterns
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              {!calibrationData ? (
                <Alert>
                  <AlertTriangle className="h-4 w-4" />
                  <AlertDescription>
                    System needs calibration. Please stand still for 10 seconds while wearing your Apple Watch, then click calibrate.
                  </AlertDescription>
                </Alert>
              ) : (
                <Alert>
              ...
              {calibrationData && (
                <div className="grid md:grid-cols-2 gap-6">
                  <div>
                    <h4 className="font-medium mb-3">Baseline Values</h4>
                    <div className="space-y-2 text-sm">
                      <div>Accelerometer: ({calibrationData.baseline.accelerometer.x.toFixed(2)}, {calibrationData.baseline.accelerometer.y.toFixed(2)}, {calibrationData.baseline.accelerometer.z.toFixed(2)})</div>
                      <div>Gyroscope: ({calibrationData.baseline.gyroscope.x.toFixed(2)}, {calibrationData.baseline.gyroscope.y.toFixed(2)}, {calibrationData.baseline.gyroscope.z.toFixed(2)})</div>
                      <div>Heart Rate: {calibrationData.baseline.heartRate.toFixed(0)} BPM</div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-2 text-sm">
                      <div>Impact: {calibrationData.thresholds.impact} m/s²</div>
                      <div>Rotation: {calibrationData.thresholds.rotation} rad/s</div>
                      <div>Heart Rate Spike: {calibrationData.thresholds.heartRateSpike} BPM</div>
                      <div>Confidence: {(calibrationData.thresholds.confidence * 100).toFixed(0)}%</div>
                    </div>
                  </div>
                </div>
              )}
                  ...
                  {fallEvents.map((event) => (
                    <div key={event.id} className="border rounded-lg p-4">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-3">
                          <Badge variant={
                            event.falseAlarm ? 'secondary' :
                            event.severity === 'critical' ? 'destructive' :
                            event.severity === 'high' ? 'destructive' :
                            'default'
                          }>
                            {event.falseAlarm ? 'False Alarm' : event.severity.toUpperCase()}
                          </Badge>
                      ...
                      <p className="text-sm text-muted-foreground">
                        {event.confirmed && !event.falseAlarm && 'Emergency contacts were notified'}
                        {event.falseAlarm && 'False alarm - user cancelled alert'}
                        {!event.confirmed && !event.falseAlarm && 'Pending confirmation'}
                      </p>
                    </div>
                  ))}
                </div>
