...
export default function HealthcarePortal({ healthData }: HealthcarePortalProps) {
  const [providers, setProviders] = useKV<HealthcareProvider[]>('healthcare-providers', [])
  const [medicalNotes, setMedicalNotes] = useKV<MedicalNote[]>('medical-notes', [])
  const [careTasks, setCareTasks] = useKV<CareTask[]>('care-tasks', [])
  
  const [selectedTab, setSelectedTab] = useState<'providers' | 'notes' | 'tasks' | 'timeline'>('providers')
  ...
  const urgentNotesCount = medicalNotes.filter(note => note.priority === 'urgent' || note.actionRequired).length
  const overdueTasks = careTasks.filter(task => task.status === 'overdue').length
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h2 className="text-2xl font-bold text-foreground">Healthcare Portal</h2>
        ...
        {(urgentNotesCount > 0 || overdueTasks > 0) && (
          <Alert className="border-orange-200 bg-orange-50">
            ...
            <AlertDescription className="text-orange-800">
              <strong>Attention Required:</strong> {urgentNotesCount} urgent medical notes and {overdueTasks} overdue tasks need your attention.
            </AlertDescription>
          </Alert>
        )}
      </div>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setNewProvider(prev => ({ ...prev, clinic: e.target.value }))}
                ...
                <div className="space-y-2">
                  ...
                  <Select 
                    ...
                    onValueChange={(value) => setNewProvider(prev => ({ ...prev, accessLevel: value as any }))}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      ...
                      <SelectItem value="limited">Limited Access - Basic metrics only</SelectItem>
                      <SelectItem value="emergency-only">Emergency Only - Critical alerts only</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex items-center gap-2 mt-3">
                          <Badge variant="secondary" className={getAccessLevelColor(provider.accessLevel)}>
                            {provider.accessLevel.replace('-', ' ')}
                          </Badge>
                          <Badge 
                            variant={provider.verificationStatus === 'verified' ? 'default' : 'outline'}
                            className={
        ...
        <div className="space-y-6">
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    >
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="recommendation">Treatment Recommendation</SelectItem>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select 
                      ...
                      onValueChange={(value) => setNewNote(prev => ({ ...prev, priority: value as any }))}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center space-x-2">
                      <Switch
                        ...
                        onCheckedChange={(checked) => setNewNote(prev => ({ ...prev, actionRequired: checked }))}
                      />
                    ...
                    <div className="flex items-center space-x-2">
                      <Switch
                        ...
                        onCheckedChange={(checked) => setNewNote(prev => ({ ...prev, patientVisible: checked }))}
                      />
                      <Label htmlFor="patient-visible">Visible to Patient</Label>
                    </div>
                  </div>
          ...
          <div className="space-y-4">
            {medicalNotes.map((note) => (
              <Card key={note.id} className={note.actionRequired ? 'border-orange-200' : ''}>
                <CardContent className="p-4">
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Track medication, exercise, and monitoring tasks assigned by your care team
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid md:grid-cols-4 gap-4 text-center">
                <div>
                  <div className="text-2xl font-bold text-blue-600">{careTasks.filter(t => t.status === 'pending').length}</div>
                  <div className="text-sm text-muted-foreground">Pending</div>
                </div>
                <div>
                  <div className="text-2xl font-bold text-yellow-600">{careTasks.filter(t => t.status === 'in-progress').length}</div>
                  <div className="text-sm text-muted-foreground">In Progress</div>
                </div>
                <div>
                  <div className="text-2xl font-bold text-green-600">{careTasks.filter(t => t.status === 'completed').length}</div>
                  <div className="text-sm text-muted-foreground">Completed</div>
                </div>
                <div>
                  <div className="text-2xl font-bold text-red-600">{careTasks.filter(t => t.status === 'overdue').length}</div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p className="text-sm">Track appointments, treatments, and progress over time</p>
              </div>
            </CardContent>
          </Card>
        </div>
