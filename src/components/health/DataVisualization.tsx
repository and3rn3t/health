    ...
    <Card className={className}>
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <Badge variant={getValueStatus(avgValue) === 'good' ? 'default' : 'secondary'}>
            Avg: {avgValue.toFixed(1)}{config.unit}
          </Badge>
        </div>
        <CardDescription>
          {timeframe.charAt(0).toUpperCase() + timeframe.slice(1)} trend over last {metricData.length} {timeframe === 'daily' ? 'days' : timeframe === 'weekly' ? 'weeks' : 'months'}
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
          ...
          <div className="relative">
            ...
            {config.target && (
              <div 
                className="absolute w-full border-t-2 border-dashed border-muted-foreground/30 z-10"
                style={{ 
              ...
              {metricData.slice(-20).map((point, index) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`
                        ${config.color} rounded-t-sm transition-all duration-200 opacity-70 hover:opacity-100
                    ...
                    <div className="absolute bottom-full mb-2 left-1/2 transform -translate-x-1/2 bg-black text-white text-xs rounded px-2 py-1 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-20">
                      {formatDate(point.date)}: {point.value.toFixed(1)}{config.unit}
                    </div>
                  </div>
                )
              })}
          ...
          <div className="grid grid-cols-3 gap-4 text-sm">
            <div className="text-center">
              ...
              <div className="font-medium">{minValue.toFixed(1)}{config.unit}</div>
            </div>
            <div className="text-center">
              <div className="text-muted-foreground">Average</div>
              <div className="font-medium">{avgValue.toFixed(1)}{config.unit}</div>
            </div>
            <div className="text-center">
              <div className="text-muted-foreground">Max</div>
              <div className="font-medium">{maxValue.toFixed(1)}{config.unit}</div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-center gap-2 text-sm">
            <div className={`w-2 h-2 rounded-full ${
              healthData.metrics[metric].trend === 'increasing' ? 'bg-green-500' :
              healthData.metrics[metric].trend === 'decreasing' ? 'bg-red-500' : 'bg-gray-500'
            }`} />
...
export function CorrelationChart({ 
  ...
  return (
    <Card className={className}>
      <CardHeader>
        <CardTitle className="text-lg">Correlation: {label1} vs {label2}</CardTitle>
      ...
      <CardContent>
        <div className="relative h-32">
          ...
          <svg className="absolute inset-0 w-full h-full">
            <polyline
              points={data1.slice(-20).map((point, index) => {
          ...
          <svg className="absolute inset-0 w-full h-full">
            <polyline
              points={data2.slice(-20).map((point, index) => {
